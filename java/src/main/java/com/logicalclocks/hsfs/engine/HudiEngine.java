...
public class HudiEngine extends FeatureGroup  {
  ...
  private FeatureGroupCommit writeHudiDataset(SparkSession sparkSession, FeatureGroup featureGroup,
    ...
    dataset.write()
        .format(Constants.HUDI_SPARK_FORMAT)
        .options(hudiArgs)
        .mode(saveMode)
        .save(featureGroup.getLocation());

    FeatureGroupCommit featureGroupCommit = getLastCommitMetadata(sparkSession, featureGroup.getLocation());
  ...
  public FeatureGroupCommit deleteRecord(SparkSession sparkSession, FeatureGroup featureGroup, Dataset<Row> deleteDF)
    ...
    deleteDF.write().format(Constants.HUDI_SPARK_FORMAT)
        .options(hudiArgs)
  ...
  // TODO (davit): this is WIP
  private void gdprDelete(SparkSession sparkSession, String instantTime, FeatureGroup featureGroup,
                           Dataset<Row> deleteDF) {
    ...
    HoodieWriteConfig config = HoodieWriteConfig.newBuilder().withPath(featureGroup.getLocation())
        .withIndexConfig(HoodieIndexConfig.newBuilder().withIndexType(HoodieIndex.IndexType.BLOOM).build()).build();
    JavaSparkContext jsc = JavaSparkContext.fromSparkContext(sparkSession.sparkContext());
    HoodieWriteClient client = new HoodieWriteClient(jsc, config);

    JavaRDD<HoodieKey> deleteRdd = deleteDF.toJavaRDD().map(row ->
        new HoodieKey(row.<String>getAs(primaryColumns.head()), row.<String>getAs(partitionColumns.head())));

    client.delete(deleteRdd, instantTime);
  }
  ...
  private Map<String, String> setupHudiWriteOpts(FeatureGroup featureGroup) throws IOException, FeatureStoreException {
    ...
    String tableName = featureGroup.getName() + "_" + featureGroup.getVersion();
    hudiArgs.put(Constants.HUDI_TABLE_NAME, tableName);
  ...
  public void registerTemporaryTable(SparkSession sparkSession, FeatureGroup featureGroup, String alias,
    ...
    Map<String, String> hudiArgs = setupHudiReadOpts(featureGroup, startTimestamp, endTimestamp);
    sparkSession.read()
        .format(Constants.HUDI_SPARK_FORMAT)
        .options(hudiArgs)
        .load(featureGroup.getLocation()).registerTempTable(alias);
  }
  ...
  private Map<String, String> setupHudiReadOpts(FeatureGroup featureGroup, Long startTimestamp, Long endTimestamp) {
    ...
    String basePath = featureGroup.getLocation();

    //Snapshot query
    if (startTimestamp == null && endTimestamp == null) {
      hudiArgs.put(Constants.HUDI_QUERY_TYPE_OPT_KEY, Constants.HUDI_QUERY_TYPE_SNAPSHOT_OPT_VAL);
      basePath =  basePath + StringUtils.repeat("/*", numberOfpartitionCols + 1);
