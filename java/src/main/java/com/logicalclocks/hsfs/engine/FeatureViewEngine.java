...
public class FeatureViewEngine {
  ...
  private List<TrainingDatasetFeature> makeLabelFeatures(Query query, List<String> labels) throws FeatureStoreException {
      ...
      for (String label : labels) {
        if (labelWithPrefixToFeature.containsKey(label)) {
          ...
          if (labelToFeatureGroups.get(label.toLowerCase()).size() > 1) {
            throw new FeatureStoreException(String.format(
                "Provided label '%s' is ambiguous and exists in more than one feature groups. You can provide the "
                    + "label with the prefix you specify in the join.", label));
          }
          trainingDatasetFeatures.add(new TrainingDatasetFeature(
          ...
          throw new FeatureStoreException(String.format(
              "Provided label '%s' do not exist in any of the feature groups.", label));
        }
