...
class TestHudiEngine:
    def test_save_hudi_fg(self, mocker):
        ...
        mock_hudi_engine_write_hudi_dataset = mocker.patch("hsfs.core.hudi_engine.HudiEngine._write_hudi_dataset")
        mock_fg_api_commit = mocker.patch("hsfs.core.feature_group_api.FeatureGroupApi.commit")
    ...
    def test_delete_record(self, mocker):
        ...
        assert mock_hudi_engine_write_hudi_dataset.call_count == 1
        assert mock_fg_api_commit.call_count == 1
        assert "hoodie.datasource.write.payload.class" in mock_hudi_engine_write_hudi_dataset.call_args.args[3]
    ...
    def test_register_temporary_table(self, mocker):
        ...
        mock_hudi_engine_setup_hudi_read_opts = mocker.patch("hsfs.core.hudi_engine.HudiEngine._setup_hudi_read_opts")
        ...
        hudi_fg_alias = hudi_feature_group_alias.HudiFeatureGroupAlias(feature_group='',
                                                                       alias=None)

        # Act
        h_engine.register_temporary_table(hudi_fg_alias=hudi_fg_alias, read_options=None)
        ...
        assert spark_session.read.format.return_value.options.return_value.load.call_args.args[0] == fg.location
        assert spark_session.read.format.return_value.options.return_value.load.return_value.createOrReplaceTempView.call_args.args[0] == hudi_fg_alias.alias

    def test_write_hudi_dataset(self, mocker):
        # Arrange
        feature_store_id = 99

        mock_hudi_engine_setup_hudi_write_opts = mocker.patch("hsfs.core.hudi_engine.HudiEngine._setup_hudi_write_opts")
        mock_hudi_engine_get_last_commit_metadata = mocker.patch("hsfs.core.hudi_engine.HudiEngine._get_last_commit_metadata")
        ...
        # Act
        h_engine._write_hudi_dataset(dataset=dataset, save_mode="test", operation=None, write_options=None)
        ...
        assert dataset.write.format.return_value.options.return_value.mode.call_args.args[0] == "test"
        assert dataset.write.format.return_value.options.return_value.mode.return_value.save.call_args.args[0] == fg.location
    ...
    def test_write_hudi_dataset(self, mocker):
        ...
        mock_hudi_engine_get_conn_str = mocker.patch("hsfs.core.hudi_engine.HudiEngine._get_conn_str")
        ...
        # Act
        result = h_engine._setup_hudi_write_opts(operation="test", write_options={"test_name": "test_value"})
    ...
    def test_write_hudi_dataset_hudi_precombine_key(self, mocker):
        ...
        # Assert
        assert mock_hudi_engine_get_conn_str.call_count == 1
        assert result == {'hoodie.bulkinsert.shuffle.parallelism': '5',
                          'hoodie.datasource.hive_sync.database': None,
                          'hoodie.datasource.hive_sync.enable': 'true',
                          'hoodie.datasource.hive_sync.jdbcurl': 'test_conn_str',
                          'hoodie.datasource.hive_sync.partition_extractor_class': 'org.apache.hudi.hive.MultiPartKeysValueExtractor',
    ...
    def test_setup_hudi_read_opts(self, mocker):
        ...
        # Act
        result = h_engine._setup_hudi_read_opts(hudi_fg_alias=hudi_fg_alias, read_options={"test_name": "test_value"})

        # Assert
        assert mock_util_get_hudi_datestr_from_timestamp.call_count == 0
        assert result == {'hoodie.datasource.query.type': 'snapshot', 'test_name': 'test_value'}
    ...
    def test_setup_hudi_read_opts_timestamp(self, mocker):
        ...
        assert result == {'hoodie.datasource.query.type': 'incremental',
    ...
    def test_get_last_commit_metadata(self, mocker):
        ...
        # Act
        result = h_engine._get_last_commit_metadata(spark_context=spark_context, base_path=None)
    ...
    def test_get_conn_str_connstr(self, mocker):
        ...
        # Act
        result = h_engine._get_conn_str() # todo this method assumes that _connstr ends with ";", but is it a fair assumption (since the generated results doesnt end with ";")?

        # Assert
        assert result == "test_url;sslTrustStore=1;trustStorePassword=4;sslKeyStore=3;keyStorePassword=4"